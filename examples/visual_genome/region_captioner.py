...
class RegionCaptioner():
  ...
  def preprocessed_image_to_descriptor(self, image, output_name='conv6'):
    net = self.image_net
    ...
    print 'mean of descriptor is %f' % np.mean(descriptor) 
    return descriptor

  def image_to_descriptor(self, image, output_name='conv6'):
    return self.preprocessed_image_to_descriptor(self.preprocess_image(image))

  
  def predict_single_word(self, descriptor, previous_word, output=['probs','predict_loc']):
    net = self.lstm_net
    ...
    image_features[:] = descriptor.reshape(image_features.shape)
    net.forward(image_features=image_features, cont_sentence=cont_input,
                input_sentence=word_input)
    output_preds={}
    for item in output:
    	output_preds[item] = np.copy(net.blobs[item].data[0, 0, :])
    return output_preds

  def predict_single_word_from_all_previous(self, descriptor, previous_words):
    for word in [0] + previous_words:
      preds = self.predict_single_word(descriptor, word)
    return preds#including probs and predict_loc
  ...
  def predict_caption_beam_search(self, descriptor, strategy, max_length=10):
    ...
    while beams_complete < len(beams):
      ...
      for beam_index, beam_log_prob, beam in \
        ...
        if beam_size == 1:
          preds = self.predict_single_word(descriptor, previous_word)
        else:
          preds = self.predict_single_word_from_all_previous(descriptor, beam)
        probs = preds['probs']
	
        pred_location = preds['predict_loc']
        #print pred_location
      ...
      new_beam_locations = \
          [beam_locations[e['prefix_beam_index']] + e['location'] for e in expansions]
      beams_complete = 0
      for beam in new_beams:
        if beam[-1] == 0 or len(beam) >= max_length: beams_complete += 1
      beams, beam_probs, beam_locations = new_beams, new_beam_probs, new_beam_locations
    #print beam_locations[0]
    #print beam_locations[-1]
    beam_locations_r = []#np.zeros((beams_n,4))
    for i,loc in enumerate(beam_locations):

      beam_locations_r.append(np.array(loc))
    #print beam_locations_r[0]
    #print beam_locations_r[-1] 
    if orig_batch_size != 1: self.set_caption_batch_size(orig_batch_size)
    return beams, beam_probs, beam_locations_r
  ...
  def score_caption(self, descriptor, caption, is_gt=True, caption_source='gt'):
    ...
    output['prob'] = []
    probs = self.predict_single_word(descriptor, 0)['probs']
    for word in caption:
      output['prob'].append(probs[word])
      probs = self.predict_single_word(descriptor, word)
    return output

  def compute_descriptors(self, image_list, output_name='conv6'):
    ...
    for batch_start_index in range(0, len(image_list), batch_size):
      ...
      print 'mean value of sample %f' % self.image_net.blobs[output_name].data[0].mean()
    #print 'mean values'
    #mean_vec = [np.mean(x) for x in descriptors]
    #print mean_vec
    return descriptors
