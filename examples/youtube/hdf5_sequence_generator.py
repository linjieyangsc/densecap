      ...
      for i in range(self.batch_num_streams):
        if not exhausted[i]:
          ...
          for name in self.substream_names:
            ...
            elif isinstance(self.streams[i][name][self.stream_indices[i]], np.ndarray) and self.streams[i][name][self.stream_indices[i]].ndim > 1:
              batch[name].resize((batch_size, self.streams[i][name][self.stream_indices[i]].shape[1],1))
              batch[name][(t*self.batch_num_streams + i), :,0] = self.streams[i][name][self.stream_indices[i]][0,:]
            else:
