...
class SequenceGenerator():
  ...
  def get_next_batch(self, truncate_at_exhaustion=True):
    ...
    for name in self.substream_names:
      sample_data = self.streams[0][name][0]
      data_len = len(sample_data) if type(sample_data) == np.ndarray else 1
      print 'data dimension: %d' % data_len
      if data_len>1:
        batch[name] = self.get_pad_value(name) * np.ones((self.batch_stream_length, 
        self.batch_num_streams,data_len), dtype=np.float32)
      else:
        batch[name] = self.get_pad_value(name) * np.ones((self.batch_stream_length,
	self.batch_num_streams), dtype=np.float32)
