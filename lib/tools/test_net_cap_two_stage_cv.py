...
if __name__ == '__main__':
    ...
    while not os.path.exists(args.caffemodel) and args.wait:
        ...
        time.sleep(10)
    sav_name = args.imdb_name + '.' + str(cfg.TEST.RPN_POST_NMS_TOP_N)
    caffe.set_mode_gpu()
    ...
    r1_range = [0.5, 0.6, 0.7, 0.8, 0.9]
    r2_range = [0.3, 0.4, 0.5, 0.6, 0.7]
    res_mat = np.zeros((5,5))
    for i,r1 in enumerate(r1_range):
        for j,r2 in enumerate(r2_range):
            cfg.TEST.RPN_NMS_THRESH = r1
            cfg.TEST.NMS = r2
            res_mat[i,j] = test_net(feature_net, embed_net, recurrent_net, imdb, \
